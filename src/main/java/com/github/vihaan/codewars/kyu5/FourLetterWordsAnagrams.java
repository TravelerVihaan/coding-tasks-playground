...
/// Today's Game
///
/// Team\_One will be Alice & Bob versus Team\_Two who are Carol & Dan, and the fun begins with a list of four-letter subject words. Going through the words on this list, the two teams compete in a game of points where one player from each team will successively take each other on one round at a time, recalling as many anagrams as possible of a subject word from their respective memories, which themselves are also lists of four-letter words.
/// A Round
///
...
///
/// [4, 4] teams are tied```
public class FourLetterWordsAnagrams {

    public static int anagrams(String[] subjects, String[][] memories, int[] players) {
    ...
    private static List<String> findAnagrams(List<String> playerMemories, char[] subjectCharArray) {
        return playerMemories.stream().filter(word -> {
            ...
            return Arrays.equals(subjectCharArray, wordCharArray);
        }).toList();
    }


    private static void validateInput(int[] players) {
        if (players.length != 2 || (players[0] > 1 && players[1] > 1) || (players[0] <= 1 && players[1] <= 1)) {
